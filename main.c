...
int main(void) {
   ...
   /* void hardware_self_test(void) {
    if (ANSELBbits.ANSELB0 != 1) {
        debug_print_str("FATAL: DAC pin not analog!\r\n");
        while(1);
    }
    
    // Vérifier la configuration du PLL
    if (!OSCCONbits.LOCK) {
        debug_print_str("PLL NOT LOCKED!\r\n");
    }
  }*/
    ...
    while(1) {
        ...
        if (TRISBbits.TRISB15 != 0) {
            debug_print_str("ERROR: AMP_ENABLE_PIN not output!\r\n");
        }
